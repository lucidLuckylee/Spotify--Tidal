...
const SpotifyCapture = {
  ...
  _addTrack(data, addedAt) {
    ...
    // One-shot: stash the first successfully-normalized track so the
    // background diagnostic can show what fields actually made it through
    // (durationMs, addedAt, explicit, ...).
    if (!this._firstTrackSample) {
      this._firstTrackSample = t;
    }
  ...
  _normalizeTrack(data, addedAt) {
    ...
    if (data.name || data.title) {
      ...
      // Try many possible ISRC paths (Spotify dropped this from web responses
      // around 2024 — kept as a no-op in case it returns).
  ...
  // (same shape it uses for `albumOfTrack.date`). Normalize to a plain string.
